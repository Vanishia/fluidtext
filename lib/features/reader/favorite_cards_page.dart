...
class _FavoriteCardsPageState extends State<FavoriteCardsPage> {
    ...
    if (mounted) {
      ...
      if (mounted) {
        setState(() {
          ...
          _cards.removeWhere((c) => !c.isFavorite);
        });
      } else {
        contextController.dispose();
        return;
  ...
  Widget _buildBody() {
    ...
    if (_cards.isEmpty) {
      return const Center(child: Text('还没有收藏卡片'));
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        return _FavoriteCardItem(
          card: _cards[index],
          favoritedAtText: _formatFavoriteAt(_cards[index].favoritedAt),
...
class _FavoriteCardItem extends StatelessWidget {
  const _FavoriteCardItem({
    required this.card,
    required this.favoritedAtText,
    required this.onCopy,
    required this.onShowContext,
  });

  final BookCard card;
  final String favoritedAtText;
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      '#${card.cardIndex} · 收藏于 $favoritedAtText',
                      maxLines: 1,
